...
# The seasonal AR model is non-linear in the measurement equation. We may additionally restrict the parameters of the model so the the process is stable at all time periods, for example by using the transformations 
...
evol_unrestr = true
if sim_stable
    ϕ(t) = restr(3*sin(2π*t/T)) # Time-varying AR coefficient
    Φ(t) = restr(0.577)
else
    ϕ(t) = 0.95*sin(2π*t/T) # Time-varying AR coefficient
    #Φ(t) = (t <= T/2 + 2*(s+1)) ? -0.5 : 0.95
    Φ(t) = 0.5 #; 0.95*(-1 + 8*(t/T₊) -8*(t/T₊)^2)
end
σₑ = 0.2
y = zeros(T₊)
paramEvol = zeros(T₊, 2)
...
p1 = plot(time, paramEvol[:,1], label = L"\phi_t", lw = 2, c = colors[2], 
    title ="parameter evolution", ylims = (-1,1), legend = :bottomleft)
plot!(time, paramEvol[:,2], label = L"\Phi_t", lw = 2, c = colors[3])
p2 = plot(time, y, label = "data", lw = 2, c = colors[1], xlabel = "time", ylabel = "", 
...
if stable
    scaler = 2*std(invrestr.(rand(Truncated(Normal(0, σ₀),-1,1), 1000)))
    scaler  = 1
    σ₀ = scaler*σ₀ 
    σᵥ = 1*σᵥ
end
...
for j = 1:nState
    plt_tmp = plot(time, paramEvol[:,j], lw = 2, c = :black, linestyle = :solid, 
        label = "true", title = titles[j], legend = legendPos[j])
    plot!(time, PGASmedian[:,j], fillrange = PGASquantiles[:,j,1],
        fillalpha = 0.2, fillcolor = :gray, label = "", lw = 0) 
    plot!(time, PGASmedian[:,j], fillrange = PGASquantiles[:,j,2],
        fillalpha = 0.2, fillcolor = :gray, label = "", lw = 0) 
    plot!(time, PGASmedian[:,j], lw = 1, c = :gray, linestyle = :solid, 
...
for j = 1:nState
    plot!(plt[j], [0;time], EKFmedian[:,j], lw = 1, c = colors[3], linestyle = :solid, 
        label = "EKF(1)")
    plot!(plt[j], [0;time], EKFquantiles[:,j,1], lw = 1, c = colors[3], 
        linestyle = :solid, label = nothing)
    plot!(plt[j], [0;time], EKFquantiles[:,j,2], lw = 1, c = colors[3], 
        linestyle = :solid, label = nothing)
end
plot(plt..., layout = (1,2), size = (800, 300), ylims = (-1.7,1.7), xlabel = "time", 
    bottommargin = 5mm)
...
for j = 1:nState
    plot!(plt[j], [0;time], UKFmedian[:,j], lw = 1, c = colors[2], linestyle = :solid, 
        label = "UKF(1)")
    plot!(plt[j], [0;time], UKFquantiles[:,j,1], lw = 1, c = colors[2], 
        linestyle = :solid, label = nothing)
    plot!(plt[j], [0;time], UKFquantiles[:,j,2], lw = 1, c = colors[2], 
        linestyle = :solid, label = nothing)
end
...
if plotIEKF
    ...
    for j = 1:nState
        plot!(plt[j], [0;time], IEKFmedian[:,j], lw = 1, c = colors[4], linestyle = :solid, 
            label = "IEKF($maxIter)")
        plot!(plt[j], [0;time], IEKFquantiles[:,j,1], lw = 1, c = colors[4], 
            linestyle = :solid, label = nothing)
        plot!(plt[j], [0;time], IEKFquantiles[:,j,2], lw = 1, c = colors[4], 
            linestyle = :solid, label = nothing)
    end
    plot(plt..., layout = (1,2), size = (800, 300), ylims = (-1.7,1.7), xlabel = "time", 
        bottommargin = 5mm)
...
if plotIEKFL
    ...
    for j = 1:nState
        ...
        plot!(plt[j], [0;time], IEKFLquantiles[:,j,1], lw = 1, c = colors[5], 
            linestyle = :solid, label = nothing)
        plot!(plt[j], [0;time], IEKFLquantiles[:,j,2], lw = 1, c = colors[5], 
            linestyle = :solid, label = nothing)
    end
    plot(plt..., layout = (1,2), size = (800, 300), ylims = (-1.7,1.7), xlabel = "time", 
        bottommargin = 5mm)
...
savefig("SAR_095_05_nostable.pdf")
